...
'''def get_correlations(KEYWORD):
    ...
    s2v = Sense2Vec().from_disk('/home/Generate_MCQ_BERT_Wordnet_Conceptnet/model/sv2/s2v_old')
    ...
    print(type(d))# <class 'spacy.tokens.span.Span'>

    assert d.text == KEYWORD # nile_river
    freq = d._.s2v_freq 
    # freq = doctext._.s2v_freq 
    ...
    print('\n\n\n')
    try: 
       closest_words = list(d._.s2v_most_similar(30)) 
    except ValueError: 
...
def get_distractors_s2v(KEYWORD):
    s2v = Sense2Vec().from_disk('/home/Generate_MCQ_BERT_Wordnet_Conceptnet/model/sv2/s2v_old')
    word=KEYWORD.lower().replace(' ','_')
    distractors = []
    ...
    for d in most_similar:
        distractors.append(d[0].split('|')[0].replace('_',' '))
    return distractors
...
for keyword in keyword_sentence_mapping:## keyword_sentence_mapping: 14쌍임
    ...
    if wordsense:
        ...
        if len(distractors) ==0:
            ...
            distractors=get_distractors_s2v(keyword)
        if len(distractors) != 0:
